...
import type { ChatSettings } from '@/shared/messages'
...
function buildLanguagePrompt(languageMode: ChatSettings['languageMode'] | undefined, pageLanguage?: string): string {
  const detectedPageLanguage = pageLanguage || 'unknown'
  if (!languageMode || languageMode === 'auto') {
    return [
      'Language guidance: respond in the same language as the user by default.',
      `If the user asks about the page and the page language is helpful, consider the detected page language: ${detectedPageLanguage}.`,
      'When the user writes in another language, answer in that language if possible.',
    ].join(' ')
  }

  return [
    `Language guidance: respond in ${languageMode}.`,
    'Keep the wording natural and concise.',
    'If the user asks for translation, preserve the original meaning and names.',
  ].join(' ')
}
...
let currentLanguageMode: ChatSettings['languageMode'] = 'auto'
...
chrome.runtime.onMessage.addListener(async (message: Message) => {
  switch (message.type) {
    ...
    case 'settings:update': {
      ...
      if (settings?.languageMode && settings.languageMode !== currentLanguageMode) {
    ...
    case 'agent:run': {
      ...
      const { tabId, userMessage, settings, pageContext, pageLanguage } = message
      ...
      const languageMode = settings?.languageMode ?? currentLanguageMode
      ...
      if (currentTabId !== tabId || !currentAgent) {
        ...
        currentAgent = new Agent({
          ...
          systemPrompt: [buildSystemPrompt(pageContext), buildLanguagePrompt(languageMode, pageLanguage)].join('\n\n'),
